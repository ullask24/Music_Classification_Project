...
TARGET_SR = 16000  # Standard-Abtastrate für Wav2Vec 2.0
DURATION = 30
FIXED_SPEC_WIDTH = 128  # Feste Bildbreite für das ResNet

# Wav2Vec 2.0 Backbone zur Extraktion der latenten Repräsentationen
...
for genre_idx, g in enumerate(GENRES):
    ...
    for filename in sorted(os.listdir(genre_folder)):
        ...
        try:
            ...
            # -----------------------------------------------------------------
            # 1. Tabellarische Features (für XGBoost)
            # -----------------------------------------------------------------
            ...
            # -----------------------------------------------------------------
            # 2. 2D Mel-Spektrogramm (für ResNet)
            # -----------------------------------------------------------------
            mel_spec = librosa.feature.melspectrogram(y=y, sr=sr, n_mels=128, n_fft=1024, hop_length=512)
            mel_spec_db = librosa.power_to_db(mel_spec, ref=np.max)
            # Normierung auf [0, 1] und Zuschneiden/Interpolieren auf feste Dimension (128, 128)
            mel_norm = (mel_spec_db - mel_spec_db.min()) / (mel_spec_db.max() - mel_spec_db.min() + 1e-8)
            mel_resized = librosa.util.fix_length(mel_norm, size=FIXED_SPEC_WIDTH, axis=1)
            spectrograms.append(mel_resized)

            # -----------------------------------------------------------------
            # 3. Wav2Vec 2.0 Latent-Sequenz (für LSTM)
            # -----------------------------------------------------------------
            waveform = torch.tensor(y, dtype=torch.float32).unsqueeze(0).to(device)
            with torch.no_grad():
                # Feature Encoder von Wav2Vec liefert zeitliche Segmente (z.B. ~50 Hz Framerate)
                latent_seq, _ = w2v_model.extract_features(waveform)
                # Letzten Layer-Output nehmen: Shape (1, Time, 768)
                rep = latent_seq[-1].squeeze(0).cpu().numpy()
                
                # Downsampling/Subsampling auf 100 Zeitschritte zur effizienten LSTM-Verarbeitung
                step = max(1, rep.shape[0] // 100)
                subsampled = rep[::step][:100]
                if subsampled.shape[0] < 100:
                    subsampled = np.pad(subsampled, ((0, 100 - subsampled.shape[0]), (0, 0)))
                sequential_features.append(subsampled)
